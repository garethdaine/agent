    ...
    <div>
                        ...
                        <div class="space-y-6">
                            <div v-for="token in tokens" :key="token.id" class="flex items-center justify-between">
                                ...
                                <div class="flex items-center ms-2">
                                    <div v-if="token.last_used_ago" class="text-sm text-muted-foreground">
                                        Last used {{ token.last_used_ago }}
                                    </div>

                                    <button
                                        v-if="availablePermissions.length > 0"
                                        class="cursor-pointer ms-6 text-sm text-muted-foreground underline"
                                        @click="manageApiTokenPermissions(token)"
                                    >
                                        Permissions
                                    </button>

                                    <button class="cursor-pointer ms-6 text-sm text-destructive" @click="confirmApiTokenDeletion(token)">
                                        Delete
                                    </button>
                                </div>
                            </div>
        ...
        <DialogModal :show="displayingToken" @close="displayingToken = false">
            ...
            <template #content>
                ...
                <div v-if="$page.props.jetstream.flash.token" class="mt-4 bg-muted px-4 py-2 rounded font-mono text-sm text-muted-foreground break-all">
