...
- Build hybrid parser with rule-based first pass, reusing existing InterrogationRunnerAdapter/AdapterFactory/Claude/Codex/SystemPromptResolver infrastructure for LLM fallback
...
- Implement async parse workflow with internal API endpoints and WebSocket completion events on user.{userId} channel
...
- Add telemetry via nl_parse_attempts table with 90-day retention, daily cleanup command, and RBAC-gated access
...
- LLM fallback reuses existing InterrogationRunnerAdapter/AdapterFactory/Claude/Codex/SystemPromptResolver infrastructure
