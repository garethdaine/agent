...
| A10 | Credential storage map (docs) + Credentials page | P2 | Done | **Docs:** `docs/plans/credential-storage-map.md` (vault, env, connectors, API tokens, security controls). **UI:** Settings → Credentials with per-provider credential listing (set/not-set badges), inline add/replace, delete, eye toggle for input. Sidebar link added |
...
| E2 | sessions_* tools (list/history/send) | P2 | Done | **API:** `GET /chat/sessions` (list with pagination, status/connector filter), `GET /chat/sessions/{id}` (show), `GET /chat/sessions/{id}/messages` (message history), `POST /chat/sessions/{id}/send` (send outbound message), `POST /chat/sessions/{id}/archive`. **UI:** Messenger → Chat History page with expandable sessions, message bubbles, send input, archive button. Sidebar link added |
