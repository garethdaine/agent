...
**Changes from existing file**: Add keys `transient_error_codes`, `non_transient_error_codes`, `ai_critic_prompt_template`, `check_profiles`. Update `default_max_parallel_tasks` default from 3 to 5. Update `capabilities_seed` to match confirmed list (replace `linting`/`file_editing` with `documentation`/`analysis`/`planning`). Remove `max_recovery_attempts` key entirely — the canonical retry-limit key is `max_retry_attempts` (int, default 3). All recovery decision logic, ContractValidator time_constraints cap formulas, and RecoveryHandler decision chains reference `config('delegation.max_retry_attempts')` exclusively. There is no separate `max_recovery_attempts` concept.
...
- `tests/Feature/ContractValidatorTest.php` — valid contracts pass, missing required_capability fails, invalid criticality enum fails, max_runtime_seconds > 86400 fails, missing verification_strategy fails, invalid check profile name fails, time_constraints cap enforcement (using `config('delegation.max_retry_attempts')`), prompt/task_markdown_path presence
...
- `tests/Feature/VerificationPipelineTest.php` — all step types execute, short-circuit on failure, evidence persistence, pipeline resumability (pending step → resolve → resume next step), AiCriticStep non-blocking flow (verifies transient job uses sentinel cron `0 0 1 1 0`), HumanApprovalStep non-blocking flow
...
### Modified Files (8)
- `config/delegation.php` — add missing keys (transient_error_codes, non_transient_error_codes, ai_critic_prompt_template, check_profiles), update capabilities_seed and default_max_parallel_tasks, remove max_recovery_attempts (consolidated into max_retry_attempts)
...
- config/delegation.php already exists with a subset of the required keys (enabled, ui_enabled, max_tasks_per_graph, max_retry_attempts, etc.) and DelegationFeatureGate middleware already exists — Phase 1 extends the config file (adding missing keys, removing max_recovery_attempts) and leaves the middleware unchanged rather than creating either from scratch.
- The canonical retry-limit config key is max_retry_attempts (int, default 3). The max_recovery_attempts key that exists in the current config/delegation.php is removed as part of Phase 1. All recovery decision logic, ContractValidator time_constraints cap formulas, and RecoveryHandler decision chains reference config('delegation.max_retry_attempts') exclusively.
- Transient AgentJob records created by AttemptSpawner and AiCriticStep use cron_expression = '0 0 1 1 0' — the project-standard sentinel cron for non-scheduled jobs matching BuildTaskRunFactory pattern. This value passes NumericCronExpression validation (it is a valid 5-part numeric cron expression) and is never triggered by the scheduler (Jan 1 at midnight, day-of-week Sunday).
- Clone endpoint is restricted to graphs in terminal statuses only (succeeded, failed, partial, cancelled). DelegationGraphPolicy::clone enforces ownership + status must be in TERMINAL_STATUSES. The locked summary baseline contained a contradiction between the endpoint description (terminal-only) and the policy description (any status) — this plan resolves it to terminal-only, consistent with the endpoint description and the failed_subtree mode semantics.
