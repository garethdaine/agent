...
7. Apply one server-side UTC clock service for all SLA windows, dedupe windows, outage windows, projection lag, and active build age calculations.
...
10. Add non-negotiable projection access invariant: projection tables are internal infrastructure data; no external ad hoc/reporting queries are permitted.
11. Enforce projection isolation via DB design: projection objects live in `agent_projection` schema, app role gets scoped access through repositories/views, reporting roles have explicit deny/revoke.
...
11. Add projection security boundary architecture: repository/services are the only approved readers of projection data; controllers and analytics integrations consume service contracts, never raw projection tables.
12. Add `active_build_age_seconds` as first-class operational signal derived from UTC now and projection build activation timestamp, exposed in API/UI/metrics.
Impacted components: `app/Services/Telemetry/*`, `app/Services/Reliability/*`, `app/Services/Cost/*`, `app/Services/DeploymentHealth/*`, `app/Services/Escalation/*`, `app/Repositories/Projection/*`, `app/Support/Time/AgentClock.php`, `config/agent.php`.
...
15. Add projection schema hardening migration sequence:
16. Create schema `agent_projection`.
17. Move projection tables/views/materialized objects into `agent_projection`.
18. Revoke default usage/select from reporting roles on `agent_projection`.
19. Grant minimal schema/table permissions only to application and projector roles.
20. Add migration assertions that raw `SELECT` privileges for reporting roles are absent on `agent_projection` relations.
21. Add projection build activation timestamps and persisted derived field support for `active_build_age_seconds` computation.
22. Migration order: workflow-key remediation, enums, ledger + append-only guard, sequence constraints, projection schema and grants hardening, projections, classifications, reliability, escalations and suppressions, overrides, cost tables, replay-build controls + unique rebuilding guard, deployment tables, final indexes and checks.
...
6. Expose canonical response fields: gate and countability states/reasons, projection lineage, terminalization metadata, observability flags, immutable `rate_card_version`, `active_projection_build_id`, row `projection_build_id`, and `active_build_age_seconds`.
...
8. Add API contract annotation that projection tables are internal and not a public query surface; all external consumers use curated API contracts only.
9. Keep runners as raw typed emitters only; ingestion, normalization, classification, scoring, costing, and escalation stay server-side.
Impacted components: `routes/api.php`, `app/Http/Controllers/Api/V1/*`, `app/Http/Requests/Agent/*`, `app/Support/Agent/*`, `app/Repositories/Projection/*`, `app/Transformers/*`.
...
7. Restrict DB role permissions so non-application reporting roles cannot query `agent_projection` directly.
Impacted components: `app/Policies/WorkflowGovernancePolicy.php`, `app/Providers/AuthServiceProvider.php`, `app/Http/Controllers/Api/V1/WorkflowGovernanceController.php`, `app/Http/Controllers/Agent/*`, `app/Models/ManualOverrideAudit.php`, `app/Models/EscalationIncident.php`, `database/migrations/*`.
...
1. Add metrics: `ingest_latency_ms`, `projection_lag_seconds`, `projection_backlog_events`, `sequence_violation_rate`, delayed/unobservable streaks, gate flip count, escalation open duration, projection high-watermark gap, and `active_build_age_seconds`.
2. Define `projection_lag_seconds` as server UTC now minus projection row `source_high_watermark_ingested_at`.
3. Define `active_build_age_seconds` as server UTC now minus active projection build `activated_at`; if no active build, value is null with explicit reason code.
4. Add replay observability metrics: active projection build id, replay mode status, parity check result, swap timestamp, and rebuild lock contention count.
5. Add append-only enforcement metric/log: blocked ledger mutation attempts by actor/service.
6. Add structured logs for classifier decisions, dedupe outcomes, gate transitions, escalation transitions, replay activation, budget enforcement, and active-build pointer changes.
7. Operator pages: `/agent/system-overview` shows ingest health, delayed vs unobservable split, lag, backlog, active build pointer, `active_build_age_seconds`, and rebuild status; `/agent/escalations` shows incident lifecycle, trigger evidence, and suppression state.
8. Alert routing supports central on-call and workflow-owner subscriptions with suppression keyed by workflow, trigger, and UTC date.
Impacted components: `app/Services/Observability/*`, `app/Services/Alerting/*`, `app/Support/Time/AgentClock.php`, `resources/js/Pages/Agent/SystemOverview/Show.vue`, `resources/js/Pages/Agent/Escalations/Index.vue`, `resources/js/Pages/Agent/ReplayBuilds/*`.
...
10. Add operator-facing projection freshness signal: show `active_build_age_seconds` on `/agent/system-overview` and `/agent/replay-builds/{buildId}` with stale-state visual indicator when threshold breached.
11. Every governance action must create visible in-page audit evidence with actor, timestamp, reason, previous/new state, and build pointer context when relevant.
...
2. Core: scheduler dispatch and pause-skip audit, run state machine, policy guardrails, telemetry ingest/schema pinning, classification/scoring, cost enforcement, escalation governance, append-only ledger enforcement, active-build read scoping, replay concurrency guard, projection schema access controls.
...
3. Publish canonical contracts: workflow-key regex, event-id scope, terminalization rules, failure taxonomy, sequence rules, UTC bucketing, trigger taxonomy, projection lag/backlog definitions, replay isolation semantics, active-build read rule, single-rebuild rule, countability transitions, pause semantics, immutable `rate_card_version` rules, projection schema boundary, and no-direct-query rule for projection tables.
4. Document `active_build_age_seconds` definition, derivation source, alert semantics, and where it appears in UI/API.
5. Require docs updates in same change set for any runtime contract change.
6. Add PR checks for docs parity, route/page discoverability impacts, additive API compatibility, trigger-taxonomy consistency, and active-build read-scope consistency.
...
10. Add projection schema transition compatibility layer: temporary compatibility views for existing internal query call sites, then remove once repository migration is complete.
...
2. Enable in dependency order: append-only ledger guard, projection schema access hardening, ingest, normalization/projections, active-build read scoping, scoring/gates, cost enforcement, deployment counting, operator surfaces, replay controls.
...
5. Pre-enable checks: enum availability, workflow-key remediation complete, incident uniqueness index valid, suppression table active, trigger taxonomy loaded, UTC clock service enabled, terminal event catalog loaded, `rate_card_version` available, single-rebuild unique index active, and projection-role grants validated.
Impacted components: `config/agent.php`, `app/Support/FeatureFlags/*`, `app/Console/Commands/*`, `app/Support/Time/*`, `database/migrations/*`.
...
2. Add unit tests for append-only guard (`UPDATE/DELETE` blocked), active-build read scoping helper, rebuild lock conflict handling, and `active_build_age_seconds` derivation.
...
4. Add feature tests for replay endpoints: cannot start rebuild when one exists, cannot activate build without parity pass, API responses always include active build pointer, and `active_build_age_seconds` appears in health/replay payloads.
5. Add DB authorization tests: reporting role cannot `SELECT` from `agent_projection` tables; app/projector roles retain required least-privilege access.
...
7. Add integration tests validating active-build-only reads under multiple historical builds and no mixed-build rows in service responses.
8. UI tests: route access and navigation discoverability, workflow widgets rendering, role-based governance visibility, audit visibility after overrides, insufficient-data rendering, delayed/unobservable indicators, escalation lifecycle transitions, gate transitions tab visibility, replay status visibility in system overview and replay builds page, and `active_build_age_seconds` visibility with stale indicator behavior.
9. Data integrity tests: immutable ledger, append-only run transitions, replay-derivable scoring/health state, projection high-watermark monotonicity, one active projection build pointer, one rebuilding build at a time.
10. CI gates: formatting, static analysis, full test suite, coverage threshold enforcement.
...
5. D5: Create `agent_projection` schema, move projection objects, and enforce role-level query restrictions for non-app/reporting roles.
6. D6: Implement ingestion dedupe, schema pinning, ingest latency capture, and UTC clock integration.
7. D7: Implement terminal event catalog, sequence violation detector, terminalization gap projector, and attempt summary terminal fields.
8. D8: Implement normalizer registry and pinned normalizer execution.
9. D9: Implement projection pipelines with lineage fields, replay-build scoping, active-build pointer state, and repository-level active-build read enforcement.
10. D10: Implement failure taxonomy enums and classification engine.
11. D11: Implement run-level scorer, stricter-window evaluator, low-volume gate behavior, countability projection, and transition source persistence.
12. D12: Implement escalation engine with incident lifecycle uniqueness by `(workflow_key, trigger_type)` and suppression table by UTC day bucket.
13. D13: Implement outage detector with delayed/unobservable split and auto-protect triggers.
14. D14: Implement pause/protect controls and scheduler paused-tick skip persistence.
15. D15: Implement assisted-SLA expiry reconciliation job.
16. D16: Implement canonical cost service, immutable rate-card pin flow, budget policy projector, and enforcement actions.
17. D17: Implement deployment registration projector with deterministic countability transitions and unresolved-incident block.
18. D18: Implement replay concurrency guard (single rebuilding build), parity-preconditioned activation flow, activation audit trail, and activation timestamp persistence.
19. D19: Implement API endpoints and validators for health, reliability, cost, escalations, pause, resume, counting, telemetry ingest, gate transitions, replay build controls, active-build status, and `active_build_age_seconds`.
20. D20: Implement web routes/pages/navigation/badges/lifecycle ribbon/discoverability checks, role-gated governance controls, and active build age display.
21. D21: Implement replay tooling, dual-write adapters, additive serializers, staging parity validator, and atomic build activation.
22. D22: Implement observability dashboards, structured logs, alert routing, lag/backlog displays, append-only guard alerts, active-build visibility, and projection-age alert hooks.
23. D23: Complete unit/feature/integration/UI/data-integrity tests with CI enforcement.
24. D24: Publish canonical docs and acceptance scorecard mapped to implemented controls.
...
3. Projection guard closure: projection tables reside in `agent_projection`, non-app/reporting roles cannot directly query them, and all served reads use scoped repositories/API.
4. Reliability closure: success contract, degraded triggers, assisted SLA expiry, run-level scoring, strict formula, stricter-window gating, insufficient-data behavior, hard-fail burst override, and gate transition history with source are active.
5. Governance closure: RBAC pause/resume authority enforced, escalation lifecycle active, one unresolved incident per workflow+trigger enforced, manual overrides persisted with required fields, and blocking-trigger taxonomy consistent.
6. Cost closure: canonical enforcement cost active, billed-cost reconciliation isolated, policy thresholds enforced, rollups persist immutable `rate_card_version` plus projection lineage.
7. Replay closure: replay runs in isolated build scope, only one rebuild can be in `rebuilding`, parity validation required before activation, and active projection swap is atomic/auditable.
8. Read-scope closure: runtime/API/UI responses use active build pointer only; no cross-build projection reads in serving paths.
9. Build freshness closure: `active_build_age_seconds` is emitted in metrics/API/UI, computed from activation timestamp via UTC clock, and visible on operator surfaces with stale indication.
10. Deployment closure: deterministic countability transitions active, unresolved blocking incident prevents counting, lifecycle states visible in control plane, optimization loop counters measurable.
11. Operator closure: required routes/pages reachable via navigation, required widgets and lifecycle ribbon visible, gate/countability/terminal/escalation/build statuses render correctly, governance controls policy-gated, and audit rows visible immediately after action.
12. Documentation closure: drift register resolved and canonical docs align with implemented contracts, replay safety semantics, incident lifecycle, route/page discoverability, active-build read scope, append-only ledger enforcement, projection no-direct-query policy, active build age signal, and positioning language.
...
- Projection-schema privilege hardening can break existing internal analytics/reporting workloads that currently query projection tables directly.
- Grant/revoke mistakes on `agent_projection` can accidentally block projector/app access or unintentionally leave reporting access open.
...
- `active_build_age_seconds` can report false freshness or false staleness if activation timestamps are missing, overwritten, or computed outside the canonical UTC clock path.
- UI discoverability can regress if navigation updates ship without route-policy alignment and click-path tests.


## Assumptions

- PostgreSQL is the source-of-record and supports required enum types, partial unique indexes, transactional DDL, triggers, constraints, and schema-level grants/revokes used by this plan.
- Application DB role model supports privilege hardening (revoke `UPDATE/DELETE` on ledger and restrict `agent_projection` reads) while preserving required inserts/selects for app/projector roles.
- Distinct reporting/analytics roles exist (or can be introduced) so projection-query restrictions can be enforced without impacting core runtime services.
...
- Server clocks are synchronized enough for deterministic UTC-based SLA checks, dedupe windows, lag metrics, suppression buckets, outage windows, and `active_build_age_seconds` computation.
- Projection build activation records include durable `activated_at` timestamps (or deterministic bootstrap values) required to derive build age consistently.
