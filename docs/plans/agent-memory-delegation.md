...
1. `enable_pgvector_extension` — `CREATE EXTENSION IF NOT EXISTS vector` wrapped in try/catch; idempotent; logs warning if unavailable
...
**Estimate: 3–4 days | Prerequisite: Phase 1 | Risk: Low**
...
**Estimate: 3–4 days | Prerequisite: Phase 2 | Risk: Medium (external dependency: laravel/ai)**
...
**Estimate: 2–3 days | Prerequisite: Phase 3 | Risk: Low**
...
**Estimate: 5–6 days | Prerequisite: Phase 3, Phase 4 | Risk: High (Neo4j + pgvector)**
...
- Retry: 5× with backoff [10, 30, 60, 120, 300]s
...
**Estimate: 3–4 days | Prerequisite: Phase 5 | Risk: Medium**
...
- Tiered pruning:
  ...
  - Vectors: composite decay scoring `importance × recency_decay × access_frequency_bonus`, prune below 0.1 threshold
...
- 3× retries, checkpoint-resumable
- Queue: `memory`
...
- 3× retries, dry-run support via constructor flag
- Queue: `memory`
...
**Estimate: 4–5 days | Prerequisite: Phase 5 | Risk: Medium**
...
- Value: `config('app.url').'/memory/v1'`
...
- `MemorySettingsController`:
  - `GET /memory/v1/settings` — read all settings (keys masked)
  - `PUT /memory/v1/settings` — update settings (batch)
  - `POST /memory/v1/settings/test-connection` — test provider connectivity + report capabilities
  - `GET /memory/v1/settings/capabilities` — operating mode + feature flags
- `MemoryCoreBlockController`:
  - `GET /memory/v1/core-blocks` — list blocks for authenticated user
  - `GET /memory/v1/core-blocks/{key}` — read single block
  - `PUT /memory/v1/core-blocks/{key}` — create/update block (self-edit)
  - `DELETE /memory/v1/core-blocks/{key}` — soft-delete block
- `MemoryRetrievalController`:
  - `POST /memory/v1/retrieve` — hybrid retrieval query
- `MemoryWorkingController`:
  - `POST /memory/v1/working/append` — append to working memory
  - `GET /memory/v1/working/{runId}` — read working memory for run
...
- **Route file**: `routes/api.php` (modify) or `routes/memory.php` (new, loaded by MemoryServiceProvider)
...
- MEMORY_API_BASE_URL injection: verify present in merged env
- HTTP API: full request/response cycle for all 12 endpoints
- Rate limiting: separate buckets enforced
- Auth: Sanctum guard, user scoping

---

## Phase 8: Integration Testing, Performance Validation, Documentation
**Estimate: 2–3 days | Prerequisite: Phase 7 | Risk: Low**
...
- Verify pgvector extension migration is truly idempotent and non-blocking
...
Phases 4 and 5 can partially overlap. Phase 6 and 7 can run in parallel after Phase 5.
...
| `routes/api.php` | Add memory/v1 route group (or separate route file) |
...
- **Critical path**: Phase 1 → 2 → 3 → 5 → 7 → 8 (18–24 days)
- **Parallelizable**: Phase 4 (alongside Phase 5), Phase 6 (alongside Phase 7)

## Sections

- Phase 1: Foundation — Config, Migrations, Models, Provider Registration (3–4 days)
- Phase 2: Core Memory (Layer 1) — CoreMemoryManager + Settings (3–4 days)
- Phase 3: Provider Abstraction — Contracts, Adapters, Rate Limiting (3–4 days)
- Phase 4: Working Memory (Layer 2) — Redis Buffer + Summarization (2–3 days)
- Phase 5: Long-term Memory (Layer 3) — Embeddings, Graph, Conversation Logs (5–6 days)
- Phase 6: Consolidation, Forgetting, Artisan Commands (3–4 days)
- Phase 7: Context Injection + HTTP API (4–5 days)
- Phase 8: Integration Testing, Performance Validation, Documentation (2–3 days)
...
- R2: pgvector extension availability — pgvector 0.8.1 may not be installable on all PostgreSQL hosting environments (permissions, shared hosting). Mitigation: System designed for graceful degradation to BM25+graph mode. Migration is idempotent and non-fatal. MemoryCapabilityResolver auto-detects.
...
- A3: pgvector 0.8.1 extension can be installed on the target PostgreSQL instance — if not, system degrades gracefully to BM25+graph mode without blocking deployment
