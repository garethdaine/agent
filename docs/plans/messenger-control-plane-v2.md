...
Schema::create('connector_accounts', function (Blueprint $table): void {
    ...
    $table->string('account_key', 64)->unique(); // deterministic routing key for multi-bot
    $table->timestamps();
    
    $table->index(['provider', 'status']);
    $table->index(['provider', 'account_key']);
});
```
...
class AccountResolver
{
    public function resolve(string $provider, Request $request): ?ConnectorAccount
    {
        ...
        if ($accountKey) {
            // Deterministic lookup by account key
...
public function extractAccountKey(Request $request): ?string
{
    // Bot token embedded in webhook URL path: /connectors/telegram/webhook/{account_key}
    return $request->route('account_key');
}
```
...
**Telegram Webhook Controller (account key in URL):**
```php
...
// Telegram: account_key in URL for routing
...
class ProgressReporter
{
    public function sendAck(
        ...
        $this->sendWithStrategy($message, $strategy, $account, $adapter);
    }
    ...
    private function sendWithStrategy(
        ...
        match ($strategy->mode) {
            ...
            'edit' => $adapter->editMessage($strategy->originalMessageId, $message, $account),
...
class QuarantineManager
{
    public function quarantine(
        ...
        // Create record with infected status
...
17. **A.17** Testing (depends on all above)
18. **B.1** Discord Adapter (depends on Phase A completion)
19. **B.2** WhatsApp Adapter (depends on Phase A completion)
20. **B.3** Discord and WhatsApp Attachments (depends on B.1, B.2)
...
- Slack team_id and Telegram account_key in URL provide reliable deterministic routing for multi-bot scenarios
