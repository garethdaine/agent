...
class CleanupExpiredPendingConfirmations implements ShouldQueue
{
    ...
    public function handle(): void
    {
        // Delete expired pending confirmations that were never confirmed or cancelled
        ...
        // Also update associated chat actions to timeout status
...
class QueueDepthMonitor
{
    ...
    private function getQueueSize(string $queue): int
    {
        ...
        // Fallback: use Horizon's pending jobs count
        return (int) \Laravel\Horizon\Contracts\MetricsRepository::class
            ? app(\Laravel\Horizon\Contracts\MetricsRepository::class)
                ->queueWithPendingJobs()[$queue] ?? 0
            : 0;
    }
}
```
