...
axiom () {
	local AXIOM_HOST="garethdaine@192.168.1.57" 
	local AXIOM_PORT="18789" 
	local TOKEN="a1c8d698fcfc49e01f726c7e328cab40e1953d081f63aa032bc6757a755e1afc" 
	case "$1" in
		(start) local EXISTING=$(timeout 5 ssh "$AXIOM_HOST" 'ps aux | grep -E "openclaw gateway|openclaw-gateway" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			if [[ $EXISTING -gt 0 ]]
			then
				echo "⚠️  OpenClaw is already running ($EXISTING process(es))"
				return 1
			fi
			echo "🚀 Starting Axiom Spark..."
			ssh "$AXIOM_HOST" 'sudo -u clawdbot /tmp/start-openclaw.sh </dev/null >/dev/null 2>&1 &' > /dev/null 2>&1
			if [[ $? -ne 0 ]]
			then
			...
			sleep 3
			local SUPERVISOR_COUNT=$(timeout 5 ssh "$AXIOM_HOST" 'ps aux | grep "openclaw$" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local GATEWAY_COUNT=$(timeout 5 ssh "$AXIOM_HOST" 'ps aux | grep "openclaw-gateway" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local COUNT=$((SUPERVISOR_COUNT + GATEWAY_COUNT)) 
			if [[ $COUNT -ge 2 ]]
			then
				echo "✅ OpenClaw started ($COUNT processes)"
			else
				echo "❌ Failed to start (only $COUNT process(es))"
				return 1
			fi
			if ! pgrep -f "ssh.*${AXIOM_PORT}" > /dev/null 2>&1
			then
				ssh -f -N -L "${AXIOM_PORT}:127.0.0.1:${AXIOM_PORT}" "$AXIOM_HOST" 2> /dev/null
				echo "✅ SSH tunnel created"
			else
				echo "ℹ️  Tunnel already exists"
			fi
			echo ""
			echo "🎉 Axiom Spark ready!"
			echo "📍 http://127.0.0.1:${AXIOM_PORT}/?token=${TOKEN}" ;;
		(stop) echo "🛑 Stopping Axiom Spark..."
			pkill -f "ssh.*${AXIOM_PORT}" 2> /dev/null
			timeout 5 ssh "$AXIOM_HOST" 'docker ps -q --filter "name=openclaw-sbx-browser" | xargs -r docker rm -f' 2> /dev/null
			timeout 5 ssh "$AXIOM_HOST" 'sudo -u clawdbot pkill -9 openclaw' 2> /dev/null
			sleep 1
			local SUPERVISOR_COUNT=$(timeout 5 ssh "$AXIOM_HOST" 'ps aux | grep "openclaw$" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local GATEWAY_COUNT=$(timeout 5 ssh "$AXIOM_HOST" 'ps aux | grep "openclaw-gateway" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local COUNT=$((SUPERVISOR_COUNT + GATEWAY_COUNT)) 
			if [[ $COUNT -eq 0 ]]
			then
				echo "✅ Stopped ($COUNT processes)"
			else
				echo "⚠️  $COUNT process(es) still running"
			fi ;;
		(restart) echo "🔄 Restarting Axiom Spark..."
			echo ""
			axiom stop
			sleep 2
			axiom start ;;
		(cleanup) echo "🧹 Cleaning up duplicate Axiom Spark processes..."
			local SUPERVISOR_COUNT=$(ssh "$AXIOM_HOST" 'ps aux | grep "openclaw$" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local GATEWAY_COUNT=$(ssh "$AXIOM_HOST" 'ps aux | grep "openclaw-gateway" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local PROCESS_COUNT=$((SUPERVISOR_COUNT + GATEWAY_COUNT)) 
			...
			if [[ $PROCESS_COUNT -eq 2 ]]
			then
				echo "  ✅ Already clean (2 processes expected: supervisor + gateway)"
				return 0
			fi
			echo "  🔪 Killing browser containers..."
			ssh "$AXIOM_HOST" 'docker ps -q --filter "name=openclaw-sbx-browser" | xargs -r docker rm -f' 2> /dev/null
			echo "  🔪 Killing all gateway processes..."
			ssh "$AXIOM_HOST" 'sudo -u clawdbot pkill -9 openclaw' 2> /dev/null
			sleep 2
			echo "  ♻️  Starting fresh instance..."
			ssh "$AXIOM_HOST" 'sudo -u clawdbot /tmp/start-openclaw.sh </dev/null >/dev/null 2>&1 &' 2> /dev/null
			sleep 3
			local NEW_SUPERVISOR=$(ssh "$AXIOM_HOST" 'ps aux | grep "openclaw$" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local NEW_GATEWAY=$(ssh "$AXIOM_HOST" 'ps aux | grep "openclaw-gateway" | grep -v grep | wc -l' 2>/dev/null | tr -d ' ') 
			local NEW_COUNT=$((NEW_SUPERVISOR + NEW_GATEWAY)) 
			if [[ $NEW_COUNT -eq 2 ]]
			then
				echo "  ✅ Cleanup complete - running 2 processes (supervisor + gateway)"
			else
				echo "  ⚠️  Running $NEW_COUNT processes (expected 2)"
			fi ;;
		(status) echo "📊 Axiom Spark Status:"
			echo ""
			local SUPERVISOR_COUNT=$(ssh "$AXIOM_HOST" 'ps aux | grep "openclaw$" | grep -v grep' 2>/dev/null | wc -l | tr -d ' ') 
			local GATEWAY_COUNT=$(ssh "$AXIOM_HOST" 'ps aux | grep "openclaw-gateway" | grep -v grep' 2>/dev/null | wc -l | tr -d ' ') 
			local TOTAL_COUNT=$((SUPERVISOR_COUNT + GATEWAY_COUNT)) 
			echo "🖥️  DGX Spark:"
			if [[ $TOTAL_COUNT -eq 0 ]]
			then
				echo "  ❌ OpenClaw not running"
			elif [[ $TOTAL_COUNT -eq 2 ]]
			then
				echo "  ✅ OpenClaw healthy (2 processes)"
			else
				echo "  ⚠️  Warning: $TOTAL_COUNT processes detected (duplicates!)"
				echo "     Run 'axiom cleanup' to fix"
			fi
			if [[ $SUPERVISOR_COUNT -gt 0 ]]
			then
				echo ""
				echo "  📋 Supervisor ($SUPERVISOR_COUNT):"
				ssh "$AXIOM_HOST" 'ps aux | grep "openclaw$" | grep -v grep | awk "{print \"     PID:\", \$2, \"| CPU:\", \$3\"%\", \"| Mem:\", \$4\"%\"}"' 2> /dev/null
			fi
			if [[ $GATEWAY_COUNT -gt 0 ]]
			then
				echo ""
				echo "  🚪 Gateway ($GATEWAY_COUNT):"
				ssh "$AXIOM_HOST" 'ps aux | grep "openclaw-gateway" | grep -v grep | awk "{print \"     PID:\", \$2, \"| CPU:\", \$3\"%\", \"| Mem:\", \$4\"%\"}"' 2> /dev/null
			fi
			echo ""
			echo "🔌 SSH Tunnel:"
			if pgrep -f "ssh.*${AXIOM_PORT}:127.0.0.1:${AXIOM_PORT}" > /dev/null
			then
				echo "  ✅ Tunnel active on port ${AXIOM_PORT}"
			else
				echo "  ❌ No tunnel"
			fi
			echo ""
			echo "💬 Discord:"
			local DISCORD_STATUS=$(ssh "$AXIOM_HOST" 'tail -5 /tmp/openclaw-gateway.log 2>/dev/null | grep "logged in to discord"' | tail -1) 
			if [[ -n "$DISCORD_STATUS" ]]
			then
				echo "  ✅ Connected"
			else
		...
		(vnc) echo "🖥️  Accessing Browser via noVNC..."
			local CONTAINER_ID=$(ssh "$AXIOM_HOST" 'docker ps --filter "name=openclaw-sbx-browser" --format "{{.ID}}"' 2>/dev/null | head -1) 
			if [[ -z "$CONTAINER_ID" ]]
			then
				echo "❌ No browser container found"
				echo "   Ask AxiomSpark to use the browser first, or run:"
				echo "   @AxiomSpark Navigate to https://example.com"
				return 1
			fi
			echo "✅ Browser container found: $CONTAINER_ID"
			if pgrep -f "ssh.*6080:" > /dev/null 2>&1
			then
				echo "ℹ️  noVNC tunnel already exists"
			else
				echo "🔌 Creating noVNC tunnel..."
				local VNC_PORT=$(ssh "$AXIOM_HOST" "docker port $CONTAINER_ID 6080 2>/dev/null | cut -d: -f2" | head -1) 
				if [[ -z "$VNC_PORT" ]]
				then
					echo "⚠️  noVNC not exposed, forwarding directly to container..."
					local CONTAINER_IP=$(ssh "$AXIOM_HOST" "docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $CONTAINER_ID") 
					ssh -f -N -L 6080:${CONTAINER_IP}:6080 "$AXIOM_HOST" 2> /dev/null
				else
					ssh -f -N -L 6080:127.0.0.1:${VNC_PORT} "$AXIOM_HOST" 2> /dev/null
				fi
				if [[ $? -eq 0 ]]
				then
					echo "✅ noVNC tunnel created"
				else
			...
			echo "   VNC:   vnc://localhost:5900 (if using VNC client)"
			echo ""
			echo "💡 Use this to manually login to X, GitHub, Gmail, etc."
			echo "   Sessions persist in the container - login once, use forever!" ;;
		(*) echo "Usage: axiom {start|stop|restart|cleanup|status|vnc}"
			echo ""
			echo "Commands:"
			echo "  start    - Start all Axiom Spark services (prevents duplicates)"
			echo "  stop     - Stop all running services"
			echo "  restart  - Restart all services"
			echo "  cleanup  - Remove duplicate processes and restart clean"
			echo "  status   - Check service status (shows all processes)"
			echo "  vnc      - Access browser via noVNC for manual logins"
			return 1 ;;
	esac
}
axiom-vnc () {
	local CONTAINER_PORT=$(ssh garethdaine@192.168.1.57 'docker ps --filter "name=openclaw-sbx-browser" --format "{{.Ports}}" | grep -o "127.0.0.1:[0-9]*->6080" | cut -d: -f2 | cut -d- -f1') 
	if [ -z "$CONTAINER_PORT" ]
	then
		echo "❌ Browser container not running"
		return 1
	fi
	ps aux | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} "ssh.*6080.*192.168.1.57" | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -v grep | awk '{print $2}' | xargs kill 2> /dev/null
	ssh -f -N -L 6080:127.0.0.1:$CONTAINER_PORT garethdaine@192.168.1.57
	echo "✅ VNC tunnel created (container port: $CONTAINER_PORT)"
...
nvwb () {
	if [[ -z "${NVWB_BIN_PATH}" ]]
	then
		NVWB_BIN_PATH="$HOME/.nvwb/bin/nvwb-cli" 
	fi
	eval_call=false 
	is_project_open_interactive=false 
	for arg in "$@"
	do
		case $arg in
			(activate | deactivate) eval_call=true 
				break ;;
			(open) eval_call=true 
				if [ "$#" -eq 1 ]
				then
					is_project_open_interactive=true 
				fi
				break ;;
			(close) eval_call=true  ;;
			(*)  ;;
		esac
	done
	last_arg="${@:$#}" 
	...
	then
		...
		if [ "$is_project_open_interactive" = true ]
		then
			if ${NVWB_BIN_PATH} internal select-project-helper
			then
				callCLI=$(PS1="${PS1:-}" NVWB_INTERACTIVE=1 NVWB_EVAL=1 NVWB_TERM_WIDTH="${term_width}" ${NVWB_BIN_PATH} open $(cat $NVWB_WORKBENCH_DIR/open-helper)) 
				ret=$? 
				eval $callCLI
				rm $NVWB_WORKBENCH_DIR/open-helper
				return $ret
			else
				return $?
			fi
			...
			if [ -t 0 ] && [ -t 1 ]
			then
				callCLI=$(PS1="${PS1:-}" NVWB_INTERACTIVE=1 NVWB_EVAL=1 NVWB_TERM_WIDTH="${term_width}" ${NVWB_BIN_PATH} "$@") 
				ret=$? 
			else
				callCLI=$(PS1="${PS1:-}" NVWB_EVAL=1 NVWB_TERM_WIDTH="${term_width}" ${NVWB_BIN_PATH} "$@") 
				ret=$? 
			fi
			eval $callCLI
			return $ret
...
alias 9='cd -9'
alias _='sudo '
alias axiom-vnc-old='axiom vnc'
alias codex-linear='codex -c "agent_instructions.mcp_gateway=\"Use gateway_search_tools and gateway_call_tool_filtered from mcp-gateway\""'
...
export CODEX_HOME=/Users/garethdaine/Code/agent/storage/framework/interrogation-codex/session-15
export COLORTERM=truecolor
export COMMAND_MODE=unix2003
export CONDA_CHANGEPS1=false
export DB_CONNECTION=pgsql_testing
export DB_DATABASE=/Users/garethdaine/Code/agent/storage/framework/interrogation-build/session-15.sqlite
...
export HERD_PHP_83_INI_SCAN_DIR='/Users/garethdaine/Library/Application Support/Herd/config/php/83/'
export HERD_PHP_84_INI_SCAN_DIR='/Users/garethdaine/Library/Application Support/Herd/config/php/84/'
export HOME=/Users/garethdaine
export HOMEBREW_CELLAR=/opt/homebrew/Cellar
export HOMEBREW_PREFIX=/opt/homebrew
export HOMEBREW_REPOSITORY=/opt/homebrew
export HORIZON_AGENT_MAX_PROCESSES=2
export INFOPATH=/opt/homebrew/share/info:/opt/homebrew/share/info:
export INTERROGATION_BUILD_DB_PATH=/Users/garethdaine/Code/agent/storage/framework/interrogation-build/session-15.sqlite
export INTERROGATION_BUILD_TASK_ID=170
export INTERROGATION_DB_ISOLATED=1
export INTERROGATION_SESSION_ID=15
export LANG=en_GB.UTF-8
export LESS='-F -X -R'
export LINEAR_CLIENT_ID=c5e1eb0ee136ae7063795b8dbe1a1af9
export LINEAR_CLIENT_SECRET=afb76b438de4a0d8023d8e105c5e4f28
export LINEAR_OAUTH_SCOPES='read write'
export LINEAR_REDIRECT_URI=https://agent.test/integrations/oauth/linear/callback
export LOGNAME=garethdaine
...
export LaunchInstanceID=4A2F1F01-11AB-4B15-BACA-CCB71287C5F8
export MAIL_FROM_ADDRESS=hello@example.com
export MAIL_FROM_NAME=Agent
...
export NVM_BIN='/Users/garethdaine/Library/Application Support/Herd/config/nvm/versions/node/v25.1.0/bin'
export NVM_CD_FLAGS=-q
export NVM_DIR='/Users/garethdaine/Library/Application Support/Herd/config/nvm'
export NVM_INC='/Users/garethdaine/Library/Application Support/Herd/config/nvm/versions/node/v25.1.0/include/node'
...
export REDIS_QUEUE_RETRY_AFTER=90000
export REVERB_APP_ID=1001
export REVERB_APP_KEY=laravel-herd
export REVERB_APP_SECRET=secret
export REVERB_HOST=reverb.herd.test
export REVERB_PORT=443
export REVERB_SCHEME=https
export SANCTUM_STATEFUL_DOMAINS=agent.test,agent.herd.test,localhost,localhost:3000,127.0.0.1,127.0.0.1:8000,::1
export SECURITYSESSIONID=186af
...
export SSH_AUTH_SOCK=/private/tmp/com.apple.launchd.4funSRVbFK/Listeners
export SSH_SOCKET_DIR='~/.ssh'
export TERM=xterm-256color
export TERM_PROGRAM=WarpTerminal
export TERM_PROGRAM_VERSION=v0.2026.02.25.08.24.stable_01
export TEST_DB_CHARSET=utf8
export TEST_DB_DATABASE=agent_test
export TEST_DB_HOST=127.0.0.1
export TEST_DB_PASSWORD=''
export TEST_DB_PORT=5432
export TEST_DB_SEARCH_PATH=public
export TEST_DB_SSLMODE=prefer
export TEST_DB_USERNAME=root
export TMPDIR=/var/folders/fl/k_3wn9md3cs4thdm3vznfrr80000gn/T/
export USER=garethdaine
export VITE_APP_NAME=Agent
export VITE_REVERB_APP_KEY=laravel-herd
export VITE_REVERB_HOST=reverb.herd.test
...
export ZSH=/Users/garethdaine/.oh-my-zsh
