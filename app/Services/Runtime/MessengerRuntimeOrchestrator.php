...
class MessengerRuntimeOrchestrator
{
    ...
    public static function browserInstructions(): string
    {
        return implode("\n", [
            ...
            '- auth save <name> --url <url> --username <u> --password <p>, auth login <name> — load credentials into browser session (session-scoped only)',
            '- tab new|list|close|<n> — tab management',
            '',
            'Selectors: Use CSS selectors or @ref IDs from snapshot output.',
            '',
            'agent-browser IS the browser sidecar. All browser commands go through it. Do NOT use raw Playwright or other browser tools.',
            '',
            'Browser Session Modes:',
            '- Ephemeral: Default. Clean browser with no saved state.',
            '- Persistent Profile ($AGENT_BROWSER_PROFILE): Cookies, localStorage, and login sessions persist across turns and restarts. Login once, reuse forever.',
            '- CDP Connection (connect <port>): Attach to the user\'s running Chrome to reuse their existing login sessions and cookies.',
            '- Auto-Connect ($AGENT_BROWSER_AUTO_CONNECT): Auto-discover and connect to a running Chrome instance.',
            '',
            'If $AGENT_BROWSER_PROFILE is set, agent-browser automatically uses the persistent profile.',
            'For CDP mode: use `agent-browser connect <port>` to attach to Chrome, then `tab list` to see existing tabs.',
            'Do NOT use the --cdp flag — it creates an isolated context without cookies. Use the `connect` command instead.',
            'For sites with bot detection (X, LinkedIn, etc.), prefer CDP (connect) mode over persistent profiles.',
            '',
            'LOGIN WORKFLOW: When asked to log in to a site, ALWAYS check the Persistent Credential Vault FIRST (see "Persistent Credential Vault" section below). Only use browser `auth save/login` to load vault credentials into the browser session — never as a standalone credential store.',
        ]);
    }
